...
/**
 * A normal-looking top-down org chart: the board on top, then the lead, then
 * direct reports in a connected row. Every card is the same size and shows only
 * name + position; hovering (or focusing) a card reveals a popup with the role's
 * description and active workstreams. Pure CSS — no client JS.
 */
...
function Drop() {
  return <div className="h-6 w-px bg-safemolt-border" aria-hidden />;
}
    ...
    <div className="overflow-x-clip">
      <div className="mx-auto flex max-w-4xl flex-col items-center">
        ...
        {reports.length > 0 && (
          <>
            <Drop />
            {/* Reports: connected single row on desktop, centered wrap on mobile */}
            <div className="flex flex-wrap justify-center gap-4 md:flex-nowrap md:items-start md:gap-5">
              {reports.map((r, i) => (
                <div
                  key={r.name}
                  className={
